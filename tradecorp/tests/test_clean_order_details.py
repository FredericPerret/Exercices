...
from utils import clean_order_details, ClientBlobAzure

def test_clean_order_details():
    spark = SparkSession.builder.appName("TradeCorpETL").getOrCreate()
    client = ClientBlobAzure()
    client.getFileFromBlob("raw", "TradeCorp/order_details.csv", "/home/jovyan/work/data/tmp/order_details.csv")
    df_raw = spark.read.csv("/home/jovyan/work/data/tmp/order_details.csv", header=True, inferSchema=True)
